...
- 0336260: Upgrade exclusively to WhatsApp Graph API v25.0. Remove runtime Graph
  version selection and legacy payload aliases, add strict v25 message and
  webhook types, and add GA clients for Groups, Marketing Messages,
  business-scoped users, In-App Signup, analytics, messaging accounts, and
  billing migrations. Runtime support is now limited to Node.js 22 and newer;
  Bun and Deno compatibility suites have been removed.
  ...
  - Webhook-embedded errors are typed as `WhatsappWebhookError` (`code`,
    `title`, `message`, `error_data`, `href`) rather than reusing
    `WhatsappError`, which wrongly required `fbtrace_id`. `WhatsappError`
    regains `error_subcode`.
  ...
  - `inAppSignup.createMessagingCustomerBase` and `getMessagingCustomerBases`
    had their response types transposed.
  - A caller-supplied `request.timeout` is no longer overwritten by the SDK
    default.

  Removed as undocumented (none of these appear in Meta's public v25 reference;
  if any turn out to be genuine partner APIs they can be restored with a
  citation):

  - `category`, `ttl_seconds`, `direct_send_config` and `messaging_account_id`
    on the messages body, and the "Direct Send" README example built on them.
  ...
  - Missing analytics filters: `phone_numbers`/`country_codes`/`product_types`
    on messaging analytics, and `metric_types`/`conversation_types`/
    `conversation_directions` on conversation analytics. Granularity is now
    typed per analytics field, matching the per-field values Meta documents.
  ...
  - `IncomingRequest`, `DownloadOptions` and `ClientOptions` are exported from
  ...
  - Inbound webhook unions (`ConversationType`, status `status`, `pricing.type`,
    ...
    strings again. Meta adds enum values without a Graph version bump, so
    closing these turns a routine upstream addition into a consumer compile
    error. Outgoing request types remain strict.
  - Malformed webhook JSON now throws `MalformedBodyWebhookError` instead of a
    raw `SyntaxError`, `WebhookError` subclasses report their own `name`, and
